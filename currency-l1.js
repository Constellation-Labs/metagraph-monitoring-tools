...
const restartCurrencyL1Nodes = async (ssmClient, event, metagraphL0NodeId) => {
  printSeparator()
  console.log('Starting initial validator l1 node')
  await startInitialValidatorNodeL1(ssmClient, event, metagraphL0NodeId, [event.ec2_instance_1_id])
  console.log('Starting initial validator l1 node finished')
  printSeparator()

  printSeparator()
  console.log('Starting validators currency L1 nodes')
  ...
  console.log('Starting validators L1 nodes finished')
  printSeparator()

  printSeparator()
  console.log('Starting to get information to join node')
  const { nodeId } = await getInformationToJoinNode(event, 'currency-l1')
  console.log('Starting to get information to join node finished')
  printSeparator()

  printSeparator()
  console.log('Check if validators started successfully')
  await checkIfValidatorsStarted(event, 'currency-l1')
  console.log('Check if validators started successfully finished')
  printSeparator()

  printSeparator()
  console.log(`Joining validators currency L1 to the cluster. GenesisNodeId: ${nodeId}`)
  await joinNodeToCluster(ssmClient, event, 'currency-l1', nodeId, [event.ec2_instance_2_id])
  await joinNodeToCluster(ssmClient, event, 'currency-l1', nodeId, [event.ec2_instance_3_id])
  console.log(`Joining validators currencyL1 to the cluster. GenesisNodeId: ${nodeId} finished`)
  printSeparator()
