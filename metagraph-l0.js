...
const startRollbackFirstNodeL0 = async (ssmClient, event, ec2InstancesIds) => {
  const l0Keys = await getKeys(ssmClient, event.ec2_instance_1_id, 'l0')
...
const restartL0Nodes = async (ssmClient, event) => {
  printSeparator()
  console.log('Starting rollback genesis l0 node')
  await startRollbackFirstNodeL0(ssmClient, event, [event.ec2_instance_1_id])
  console.log('Starting rollback genesis l0 node finished')
  printSeparator()

  printSeparator()
  console.log('Starting validators L0 nodes')

  const validator1Keys = await getKeys(ssmClient, event.ec2_instance_2_id, 'l0')
  ...
  const validator2Keys = await getKeys(ssmClient, event.ec2_instance_3_id, 'l0')
  await startValidatorNodeL0(
    ssmClient,
    event,
    [
  ...
  console.log('Starting validators L0 nodes finished')
  printSeparator()

  printSeparator()
  console.log('Starting to get information to join node')
  const { nodeId } = await getInformationToJoinNode(event, 'l0')
  console.log('Starting to get information to join node finished')
  printSeparator()

  printSeparator()
  console.log('Check if validators started successfully')
  await checkIfValidatorsStarted(event, 'l0')
  console.log('Check if validators started successfully finished')
  printSeparator()

  printSeparator()
  console.log(`Joining validators L0 to the cluster. GenesisNodeId: ${nodeId}`)
  await joinNodeToCluster(ssmClient, event, 'l0', nodeId, [event.ec2_instance_2_id])
  await joinNodeToCluster(ssmClient, event, 'l0', nodeId, [event.ec2_instance_3_id])
  console.log(`Joining validators L0 to the cluster. GenesisNodeId: ${nodeId} finished`)
  printSeparator()
